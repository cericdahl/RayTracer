...
window_radius = 3*2.54;
tunnel_outer_radius = 5*2.54;
window_inside = -14*2.54 - .254; % 0.1" for seal
window_thickness = .9*2.54;
chamfer_vertex = -15.81*2.54;
reflector_bottom = cylinder_bottom-14;
reflector_bottom_minor_radius = 9.4;
reflector_bottom_hole_depth = 12.5;
...
cam_x = lens_x;
cam_z = lens_z+.18;
cam_y = window_inside - window_thickness - lens_minthick - 1.5;
cam_focal_length = .35;
...
surface_list(end).inbounds_function = @(p)(reshape( ((p(:,1,:).^2 + p(:,3,:).^2)<=(window_radius^2)) & ...
    ((((p(:,1,:)-lens_x).^2 + (p(:,3,:)-lens_z).^2))>(lens_cylrad^2)) & ...
    ((((p(:,1,:)+lens_x).^2 + (p(:,3,:)-lens_z).^2))>(lens_cylrad^2)) & ...
    (((p(:,1,:).^2 + (p(:,3,:)-fiber_lens_z(1)).^2))>(fiber_lens_cylrad^2)) & ...
    (((p(:,1,:).^2 + (p(:,3,:)-fiber_lens_z(2)).^2))>(fiber_lens_cylrad^2)), ...
    size(p,1), [] ));
...
surface_list(end+1).description = 'glass - lense interface (plane)';
surface_list(end).intersect_function = @(sp,indir)RayToPlane(sp,indir, ...
    [0 window_inside-window_thickness 0], [0 -1 0]);
surface_list(end).inbounds_function = @(p)(reshape( ...
    ...
    (((p(:,1,:).^2 + (p(:,3,:)-fiber_lens_z(2)).^2))<=(fiber_lens_cylrad^2)), ...
    size(p,1), [] ));
surface_list(end).n_outside = n_lens;
surface_list(end).n_inside = n_glass;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;

surface_list(end+1).description = 'lens1 - air interface (cyl)';
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [-lens_x 0 lens_z], [0 1 0], lens_cylrad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    (p(:,2,:)>(window_inside-window_thickness-lens_minthick-lens_rad+sqrt(lens_rad^2-lens_cylrad^2))) & ...
    (p(:,2,:)<(window_inside-window_thickness)), size(p,1), [] ));
surface_list(end).n_outside = n_air;
surface_list(end).n_inside = n_lens;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;

surface_list(end+1).description = 'lens2 - air interface (cyl)';
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [lens_x 0 lens_z], [0 1 0], lens_cylrad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    (p(:,2,:)>(window_inside-window_thickness-lens_minthick-lens_rad+sqrt(lens_rad^2-lens_cylrad^2))) & ...
...
surface_list(end+1).description = 'fiber lens1 - air interface (cyl)';
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [0 0 fiber_lens_z(1)], [0 1 0], fiber_lens_cylrad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    (p(:,2,:)>(window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad+sqrt(fiber_lens_rad^2-fiber_lens_cylrad^2))) & ...
    (p(:,2,:)<(window_inside-window_thickness)), size(p,1), [] ));
surface_list(end).n_outside = n_air;
surface_list(end).n_inside = n_lens;
surface_list(end).surface_type = 'normal';
...
surface_list(end+1).description = 'fiber lens2 - air interface (cyl)';
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [0 0 fiber_lens_z(2)], [0 1 0], fiber_lens_cylrad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    (p(:,2,:)>(window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad+sqrt(fiber_lens_rad^2-fiber_lens_cylrad^2))) & ...
    (p(:,2,:)<(window_inside-window_thickness)), size(p,1), [] ));
surface_list(end).n_outside = n_air;
...
surface_list(end+1).description = 'lens1 - air interface (sph)';
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [-lens_x (window_inside-window_thickness-lens_minthick-lens_rad) lens_z], lens_rad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    p(:,2,:)>(window_inside-window_thickness-lens_minthick-lens_rad+sqrt(lens_rad^2-lens_cylrad^2)), size(p,1), [] ));
surface_list(end).n_outside = n_lens;
surface_list(end).n_inside = n_air;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;

surface_list(end+1).description = 'lens2 - air interface (sph)';
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [lens_x (window_inside-window_thickness-lens_minthick-lens_rad) lens_z], lens_rad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    p(:,2,:)>(window_inside-window_thickness-lens_minthick-lens_rad+sqrt(lens_rad^2-lens_cylrad^2)), size(p,1), [] ));
...
surface_list(end+1).description = 'fiber lens1 - air interface (sph)';
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [0 (window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad) fiber_lens_z(1)], fiber_lens_rad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    p(:,2,:)>(window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad+sqrt(fiber_lens_rad^2-fiber_lens_cylrad^2)), size(p,1), [] ));
surface_list(end).n_outside = n_lens;
surface_list(end).n_inside = n_air;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;

surface_list(end+1).description = 'fiber lens2 - air interface (sph)';
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [0 (window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad) fiber_lens_z(2)], fiber_lens_rad);
surface_list(end).inbounds_function = @(p)(reshape( ...
    p(:,2,:)>(window_inside-window_thickness-fiber_lens_minthick-fiber_lens_rad+sqrt(fiber_lens_rad^2-fiber_lens_cylrad^2)), size(p,1), [] ));
surface_list(end).n_outside = n_lens;
surface_list(end).n_inside = n_air;
...
surface_list(end).inbounds_function = @(p)(reshape( ((p(:,3,:)<=reflector_bottom) & ...
    ((p(:,1,:).^2 + p(:,2,:).^2) > window_radius^2) ), size(p,1), [] ));
surface_list(end).n_outside = inf;
surface_list(end).n_inside = n_glycol;
surface_list(end).surface_type = 'retro';
surface_list(end).absorption = 1;
...
%     ((p(:,1,:).^2 + p(:,3,:).^2) > ((.5*chamfer_vertex*(1-sqrt(2*(vessel_radius/chamfer_vertex)^2 - 1)))^2)), ...
%     size(p,1), [] ));
surface_list(end).inbounds_function = @(p)(reshape( ((p(:,2,:)>0) | ...
    ((p(:,1,:).^2 + p(:,3,:).^2) > (tunnel_outer_radius^2))) & ...
...
surface_list(end+1).description = 'Fiber ends (plane)';
surface_list(end).intersect_function = @(sp,indir)RayToPlane(sp,indir, ...
    [0 fiber_y 0], [0 -1 0]);
surface_list(end).inbounds_function = @(p)(reshape( ...
    (((p(:,1,:).^2 + (p(:,3,:)-fiber_lens_z(1)).^2))<=(fiber_cylrad^2)) | ...
    (((p(:,1,:).^2 + (p(:,3,:)-fiber_lens_z(2)).^2))<=(fiber_cylrad^2)), ...
    size(p,1), [] ));
surface_list(end).n_outside = n_lens;
surface_list(end).n_inside = n_glass;
surface_list(end).surface_type = 'normal';
...
surface_list(end+1).description = 'reflector hole tunnel';
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [0 0 0], [0 0 1], window_radius);
surface_list(end).inbounds_function = @(p)(reshape( (p(:,3,:)>=(reflector_bottom-reflector_bottom_minor_radius-reflector_bottom_hole_depth)) & ...
    (p(:,3,:) < reflector_bottom) & ...
    (( ((p(:,1,:).^2+p(:,2,:).^2)./(vessel_radius^2)) + ((p(:,3,:)-reflector_bottom).^2./(reflector_bottom_minor_radius^2)) )>=1), size(p,1), [] ));
surface_list(end).n_inside = n_glycol;
surface_list(end).n_outside = inf;
surface_list(end).surface_type = 'retro';
surface_list(end).absorption = 1;

surface_list(end+1).description = 'reflector hole bottom';
surface_list(end).intersect_function = @(sp,indir)RayToPlane(sp,indir, ...
    [0 0 reflector_bottom-reflector_bottom_minor_radius-reflector_bottom_hole_depth], [0 0 -1]);
surface_list(end).inbounds_function = @(p)(reshape( ((p(:,1,:).^2 + p(:,2,:).^2) < window_radius^2), ...
    size(p,1), [] ));surface_list(end).n_outside = inf;
surface_list(end).n_inside = n_glycol;
surface_list(end).surface_type = 'retro';
surface_list(end).absorption = 1;
...
ray_startingpoints{2} = repmat([cam_x cam_y cam_z],size(raydirections,1),1);
