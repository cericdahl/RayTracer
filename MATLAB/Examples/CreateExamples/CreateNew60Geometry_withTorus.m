...
function [surface_list rays ray_startingpoints pixels] = CreateNew60Geometry_withTorus(geospecs)
...
jar_thick = .25; % thickness of cylinder wall
jar_cylrad = 15; % outer radius of cylinder
jar_axrad = 30; % outer radius of sphere (along cylinder axis)
jar_knucklerad = 5;
...
fid_mark_z = [-35.3, -46.8, -54.9, -64, -75] + 40;
fid_mark_rphi = 32; % distance around circumference to back points
fid_mark_rphi2 = 0; % distance around circumference to front points
...
hemi_vol = pi * ( ...
    quad(@(z)(r3(2)^2-(d(2)-z).^2), -r3(2)+d(2), z(2)) + ...
    quad(@(z)((r1(2)-r2(2)+sqrt(r2(2).^2-z.^2)).^2), z(2), 0) );

liquid_level = ( (target_mass / target_density) - hemi_vol ) / ...
    (pi*(jar_cylrad-jar_thick)^2);
...
jar_rotmat = [cos(vp_phi*.5) -sin(vp_phi*.5) 0 ; sin(vp_phi*.5) cos(vp_phi*.5) 0; 0 0 1];

fid_mark_phi = fid_mark_rphi / jar_cylrad;
fid_mark_phi2 = fid_mark_rphi2 / jar_cylrad;
...
% quartz_hemi_inside_Q = [(jar_cylrad-jar_thick)^-2 0 0 ; 0 (jar_cylrad-jar_thick)^-2 0 ; 0 0 (jar_axrad-jar_axthick)^-2 ];
% quartz_hemi_outside_Q = [(jar_cylrad)^-2 0 0 ; 0 (jar_cylrad)^-2 0 ; 0 0 (jar_axrad)^-2 ];

quartz_upperhemi_inside_Q = [(jar_cylrad-jar_thick)^-2, 0, 0 ; 0, (jar_cylrad-jar_thick)^-2, 0 ; 0, 0, (jar_axrad_top-jar_axthick_top)^-2];
...
surface_list(end).intersect_function = @(sp,indir)RayToCylinder(sp,indir, ...
    [0 0 0], [0 0 1], jar_cylrad - jar_thick);
...
surface_list(end).inbounds_function = @(p)(reshape( ...
    ...
    (abs(reshape(abs(atan2( real(sum( (reshape(permute(p,[1 3 2]),[],3)*jar_rotmat) .* repmat([1 0 0],size(p,1)*size(p,3),1), 2)), ...
    real(sum( (reshape(permute(p,[1 3 2]),[],3)*jar_rotmat) .* repmat([0 -1 0],size(p,1)*size(p,3),1), 2)) )), size(p,1), 1, []) - fid_mark_phi2)<fid_mark_penphi) ) & ...
    ( (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(1))<fid_mark_length) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(3))<fid_mark_length) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(4))<fid_mark_length) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(5))<fid_mark_length) | ...
    ...
    (abs(reshape(abs(atan2( real(sum( (reshape(permute(p,[1 3 2]),[],3)*jar_rotmat) .* repmat([1 0 0],size(p,1)*size(p,3),1), 2)), ...
    real(sum( (reshape(permute(p,[1 3 2]),[],3)*jar_rotmat) .* repmat([0 -1 0],size(p,1)*size(p,3),1), 2)) )), size(p,1), 1, []) - fid_mark_phi2)<fid_mark_lengthphi) ) & ...
    ( (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(1))<fid_mark_pen) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(3))<fid_mark_pen) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(4))<fid_mark_pen) | ...
    (abs(sum(p.*repmat([0 0 1]*(jar_rotmat'),[size(p,1) 1 size(p,3)]),2)-fid_mark_z(5))<fid_mark_pen) | ...
...
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [0 0 d(2)], jar_axrad-jar_thick);
surface_list(end).inbounds_function = @(p)(reshape( (p(:,3,:)<z(2)), size(p,1), [] ));
surface_list(end).n_outside = n_jar;
surface_list(end).n_inside = n_target;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;
...
surface_list(end).intersect_function = @(sp,indir)RayToSphere(sp,indir, ...
    [0 0 d(1)], jar_axrad);
surface_list(end).inbounds_function = @(p)(reshape( (p(:,3,:)<z(1)), size(p,1), [] ));
surface_list(end).n_outside = n_hydraulic;
surface_list(end).n_inside = n_jar;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;
...
surface_list(end).intersect_function = @(sp,indir)RayToTorus(sp,indir, ...
    [0 0 0], [0 0 1], jar_cylrad-jar_knucklerad, jar_knucklerad-jar_thick);
surface_list(end).inbounds_function = @(p)(reshape( (p(:,3,:)<0 & p(:,3,:)>=z(2) & (p(:,1,:).^2+p(:,2,:).^2)>((jar_cylrad-jar_knucklerad)^2) ), size(p,1), [] ));
surface_list(end).n_outside = n_jar;
surface_list(end).n_inside = n_target;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;
...
surface_list(end).intersect_function = @(sp,indir)RayToTorus(sp,indir, ...
    [0 0 0], [0 0 1], jar_cylrad-jar_knucklerad, jar_knucklerad);
surface_list(end).inbounds_function = @(p)(reshape( (p(:,3,:)<0 & p(:,3,:)>=z(1) & (p(:,1,:).^2+p(:,2,:).^2)>((jar_cylrad-jar_knucklerad)^2) ), size(p,1), [] ));
surface_list(end).n_outside = n_hydraulic;
surface_list(end).n_inside = n_jar;
surface_list(end).surface_type = 'normal';
surface_list(end).absorption = 0;
...
surface_list(end).inbounds_function = @(p)(reshape( (p(:,1,:).^2 + p(:,2,:).^2) < ((jar_cylrad-jar_thick)^2), size(p,1), [] ));
...
surface_list(end).inbounds_function = @(p)(reshape( ( ...
    (p(:,3,:) < pv_top) & ...
    (p(:,3,:) > jar_cyllength) & ...
    (( (p(:,1,:).^2 + p(:,2,:).^2)*(jar_cylrad-jar_thick)^-2 + (p(:,3,:)-jar_cyllength).^2*(jar_axrad_top-jar_axthick_top)^-2 ) >= 1 ) ), ...
    size(p,1), []));
