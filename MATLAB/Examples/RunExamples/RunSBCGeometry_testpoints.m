...
%         cumpos_ix = [0 ; cumsum(n_rays_by_pos)];
        ...
        n_rays = 1e4;%cumpos_ix(end);
        ray_startingpoints = repmat(pos_list(i_p,:),n_rays,1);%zeros(n_rays, 3);
...
%             
%             if 0 && (thisgamma(i_pos,4) == 0) && (thisgamma(i_pos, 7) > 0.03456)
