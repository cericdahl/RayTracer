...
def createGeometry(gs):
    ...
    head_in_P = np.array([0, 0, -2*gs.pv_top* ((gs.pv_axrad-.gspv_thick)**(-2))])
    ...
    [raydirections, pixelmap] = CreateSomeLightRays(gs.cam_resolution, cam_pixelpitch, .5*(1+gs.cam_resolution), gs.cam_f, gs.cam_pitch + gs.vp_theta - (np.pi/2), gs.cam_yaw, gs.cam_roll, gs.cam_barreld, gs.cam_lenstype)
    ...
    ray_startingpoints.append(np.matlib.repmat(vp_center + [gs.cam_x, 0, 0] + gs.cam_z*vp_axis + gs.cam_y*np.cross(vp_axis, [1,0,0]),(np.size(raydirections,0),1)))
