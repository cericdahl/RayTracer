...
def RefractionReflectionAtInterface(incoming_rays, surface_normals, n1, n2, tir_handling = np.float64(-1):
    ...
    # if np.size(n1)==1:  # Unnecessary
    #     n1 = np.matlib.repmat(n1,np.size(incoming_rays,0),1)  # Unnecessary
    # else:
    #     n1 = n1[:]  #Unnecessary

    # if np.size(n2)==1:  # Unnecessary
    #     n2 = np.matlib.repmat(n2,np.size(incoming_rays,0),1)  # Unnecessary
    # else:
    #     n2 = n2[:]  #Unnecessary


    if np.size(tir_handling)==1:
        tir_handling = tir_handling + np.zeros(np.size(incoming_rays,0))
    #     tir_handling = np.matlib.repmat(tir_handling,np.size(incoming_rays,0),1)  # Unnecessary
    # else:
    ...
    if np.any(goodray_cut):
        incoming_rays[goodray_cut,0:3] = incoming_rays[goodray_cut,0:3] / np.abs(np.sqrt(np.sum(incoming_rays[goodray_cut,0:3]**2,
                                                                                                axis=1, keepdims=True)))


    goodsurface_cut = np.sum(surface_normals**2,1) > 0
    if np.any(goodsurface_cut):
        surface_normals[goodsurface_cut,:] = surface_normals[goodsurface_cut,:] / np.abs(np.sqrt(np.sum(surface_normals[goodsurface_cut,:]**2,
                                                                                                        axis=1, keepdims=True)))

    incoming_rays[:,3:6] = incoming_rays[:,3:6] - np.sum(incoming_rays[:,3:6]*incoming_rays[:,0:3], axis=1, keepdims=True) * incoming_rays[:,0:3]

    goodpolarization_cut = np.sum(incoming_rays[:,3:6]**2,1) > 0
    if np.any(goodpolarization_cut):
        incoming_rays[goodpolarization_cut,3:6] = incoming_rays[goodpolarization_cut,3:6] / np.abs(np.sqrt(np.sum(incoming_rays[goodpolarization_cut,3:6]**2,
                                                                                                                  axis=1, keepdims=True)))
    ...
    sin_incident_angle = np.abs(np.sqrt(np.sum(interface_normals**2, axis=1)))
    ...
    # % but instead we do it one element at a time
        old_polarization = incoming_rays[goodinterface_cut,7:10]
    ...
    amplitudes = np.zeros((incoming_rays.shape[0],3,2), dtype=np.complex128)
    ...
    sin_refracted_angle = np.complex128(sin_incident_angle * n1 / n2)
    cos_refracted_angle = np.sqrt(1 - sin_refracted_angle**2)

    rsp = np.zeros((np.size(sin_refracted_angle), 2), dtype=np.complex128)
    rsp[:,0] = (n1*cos_incident_angle - n2*cos_refracted_angle) / (n1*cos_incident_angle + n2*cos_refracted_angle)
    rsp[:,1] = -(n1*cos_refracted_angle - n2*cos_incident_angle) / (n1*cos_refracted_angle + n2*cos_incident_angle)

    rsp[np.logical_or(n2==np.inf, n2==-np.inf), 0] = -1
    rsp[np.logical_or(n2==np.inf, n2==-np.inf), 1] = 1

    tsp = np.abs(np.sqrt(1-np.conj(rsp)*rsp)) # this is float64 now

    refracted_amplitudes = amplitudes * tsp[:, np.newaxis, :]
    reflected_amplitudes = amplitudes * rsp[:, np.newaxis, :]

    # %% get back to stokes parameters
    if np.any(goodhit_cut):
        refracted_rays[goodhit_cut,6] = np.real(np.sum(np.sum(np.conj(refracted_amplitudes[goodhit_cut,:,:]) * refracted_amplitudes[goodhit_cut,:,:], axis=2), axis=1))
        refracted_rays[goodhit_cut,7] = np.real((-np.sum(np.diff(np.conj(refracted_amplitudes[goodhit_cut,:,:]) * refracted_amplitudes[goodhit_cut,:,:], n=1, axis=2), axis=1))[:,0])
        refracted_rays[goodhit_cut,8] = np.real(np.sum(np.real(2 * np.conj(refracted_amplitudes[goodhit_cut,:,0]) * refracted_amplitudes[goodhit_cut,:,1]), axis=1))
        refracted_rays[goodhit_cut,9] = np.real(np.sum(np.imag(2 * np.conj(refracted_amplitudes[goodhit_cut,:,0]) * refracted_amplitudes[goodhit_cut,:,1]), axis=1))

        reflected_rays[goodhit_cut,6] = np.real(np.sum(np.sum(np.conj(reflected_amplitudes[goodhit_cut,:,:]) * reflected_amplitudes[goodhit_cut,:,:], axis=2), axis=1))
        reflected_rays[goodhit_cut,7] = np.real((-np.sum(np.diff(np.conj(reflected_amplitudes[goodhit_cut,:,:]) * reflected_amplitudes[goodhit_cut,:,:], n=1, axis=2), axis=1))[:,0])
        reflected_rays[goodhit_cut,8] = np.real(np.sum(np.real(2 * np.conj(reflected_amplitudes[goodhit_cut,:,0]) * reflected_amplitudes[goodhit_cut,:,1]), axis=1))
        reflected_rays[goodhit_cut,9] = np.real(np.sum(np.imag(2 * np.conj(reflected_amplitudes[goodhit_cut,:,0]) * reflected_amplitudes[goodhit_cut,:,1]), axis=1))
    ...
    if np.any(goodcut):
        new_yaxis[goodcut,:] = new_yaxis[goodcut,:] / np.abs(np.sqrt(np.sum(new_yaxis[goodcut,:]**2, axis=1, keepdims=True)))
    ...
    total_internal_reflection_cut = np.logical_and(goodcut, (np.real(sin_refracted_angle) >= 1))
    refracted_cut = np.logical_and(goodcut, ~total_internal_reflection_cut)

    if np.any(refracted_cut):
        refracted_rays[refracted_cut, 0:3] = -cos_refracted_angle[refracted_cut,np.newaxis] * surface_normals[refracted_cut,:] - sin_refracted_angle[refracted_cut,np.newaxis] * new_yaxis[refracted_cut,:]

    if np.any(total_internal_reflection_cut):
        if np.any(np.logical_and(total_internal_reflection_cut, (tir_handling<0))):
            #print(refracted_rays.shape)
            #print(new_yaxis.shape)
            #print(refracted_rays[np.logical_and(total_internal_reflection_cut, (tir_handling<0)[:,0]), 0:3].shape)
            refracted_rays[np.logical_and(total_internal_reflection_cut, (tir_handling<0)), 0:3] = -new_yaxis[np.logical_and(total_internal_reflection_cut, (tir_handling<0)),:]
